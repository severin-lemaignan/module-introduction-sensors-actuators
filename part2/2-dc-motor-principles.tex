...
\end{frame}
\videoframe{part3/figs/homopolarmotor.ogv?start=1}
...
\begin{frame}{End view of coil in magnetic field}
...
\only<3>{
If current $I$ flows through coil of depth $L$ and it is in magnetic field $B$,
coil pivots so it can rotate around its centre. 

Then torque $\tau$ exerted of coil is given by:
...
{\small
Distance between coil sides = $d$; angle to horizontal = $\theta$
}
                ...
                \resizebox{\linewidth}{!}{
                    \begin{tikzpicture}[>=latex]
                        ...
                        \coordinate (anchor) at (-1.5,0);
                            ...
                            \node at (0,-1) (caption) {\footnotesize copper
                            commutator} edge[bend left,thick,->] (anchor);
                    \end{tikzpicture}
                }
...
\begin{frame}{DC Motor Torque Ripple}
    ...
    \begin{center}
        ...
        \video[1]{0.35\linewidth}{part3/figs/torque-ripple.mp4?autostart&loop}
    \end{center}
...
\item $Ke$ = Electromotive force constant (in V/rad/sec) % CHECK
\end{itemize}
}
    \only<2> {
...
\imageframe[caption=Typical brushless motor]{image81}
